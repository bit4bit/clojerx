(ns MinimalExampleRoot
  (:require [MinimalExampleShare :as t])
  (:gen-class))
...
(defn first-month-of-year [year]
  (t/first-month-of-year year))

(def dispatch
  {"first-month-of-year" first-month-of-year})

(defn -main [& args]
  (let [node (OtpNode. "MinimalExampleRoot")
        mbox (.createMbox node)]
    (.registerName mbox "MinimalExampleRoot")
    (loop []
